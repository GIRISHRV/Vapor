...
class _LockScreenOverlayState extends State<LockScreenOverlay> {
  ...
  Future<void> _checkExistingPins() async {
    ...
    // Auto-migrate legacy plaintext PINs to hashes
    if (real != null && real.length == 4) {
      real = sha256.convert(utf8.encode(real)).toString();
      await prefs.setString('vapor_real_pin', real);
    }
    if (duress != null && duress.length == 4) {
      duress = sha256.convert(utf8.encode(duress)).toString();
      await prefs.setString('vapor_duress_pin', duress);
    }

    if (real == null || duress == null) {
      setState(() {
        _isSetupMode = true;
        _isLoading = false;
      });
    ...
    if (_isSetupMode) {
      if (_setupStep == 0) {
        ...
        if (_input == _tempSetupPin) {
          _duressPin = _input;
          final prefs = await SharedPreferences.getInstance();
          final hashedReal = sha256.convert(utf8.encode(_realPin)).toString();
          final hashedDuress = sha256
              .convert(utf8.encode(_duressPin))
              .toString();
      ...
      // Verification Mode — compare hashes
